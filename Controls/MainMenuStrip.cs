...
namespace NotePad.Controls
{
    public class MainMenuStrip : MenuStrip
    {
        ...
        {
            var formatDropDown = new ToolStripMenuItem("Edition");
        ...
        public void ViewDropDownMenu()
        {
            ...
            var zoomRestore = new ToolStripMenuItem("Restaurer le zoom par défaut", null, null, Keys.Control | Keys.Divide);

            zoomIn.ShortcutKeyDisplayString = "Ctrl+Num +";
            zoomOut.ShortcutKeyDisplayString = "Ctrl+Num -";
            zoomRestore.ShortcutKeyDisplayString = "Ctrl+Num /";
            ...
            zoomOut.Click += (s, e) =>
            {
                if (_form.CurrentRtb.ZoomFactor > 0.5F)
                {
                    _form.CurrentRtb.ZoomFactor -= 0.3F;
                }
            };

            zoomRestore.Click += (s, e) => { _form.CurrentRtb.ZoomFactor = 1F; };

            zoomDropDown.DropDownItems.AddRange(new ToolStripItem[] { zoomIn, zoomOut, zoomRestore});
